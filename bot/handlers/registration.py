...
async def start(message: types.Message):
    ...
    if len(command_with_args) < 2:
        await message.answer(
            "👋 Привет! Этот бот предназначен для отправки уведомлений участникам хакатонов."
        )
        return
    ...
    if not user_uuid:
        await message.answer(
            "Упс! 😅 Кажется, не удалось найти тебя. Попробуй еще раз!"
        )
        return
    ...
    try:
        response = await token_manager.request_with_token(
            ...
            },  # Передаем как параметры запроса
        ...
        if response.status_code == 200:
            ...
            await message.answer(
                "Ой! 😕 Что-то пошло не так... Попробуй еще раз чуть позже."
            )
